...
{
    DWORD cs2_cheat_controller::getCs2ProcessId()
    {
    ...
    uintptr_t cs2_cheat_controller::getClientDllBase() const
    {
    ...
    cs2_cheat_controller::cs2_cheat_controller(): m_driver(driver::driver{}), m_cs2_process_id(getCs2ProcessId()), m_client_dll_base(getClientDllBase())
    {
    ...
    {
        const auto entityControllerPawn{m_driver.read<uintptr_t>(entity_controller + cs2_dumper::schemas::client_dll::CCSPlayerController::m_hPlayerPawn)};
        if (!entityControllerPawn)
        {
            return std::nullopt;
        }

        const auto listEntity = m_driver.read<uintptr_t>(entity_system + (0x8 * ((entityControllerPawn & 0x7FFF) >> 9) + 0x10));
        if (!listEntity)
        {
            return std::nullopt;
        }
    ...
    int cs2_cheat_controller::get_highest_entity_index(const uintptr_t& entity_system) const
    {
        return m_driver.read<int>(entity_system + cs2_dumper::offsets::client_dll::dwGameEntitySystem_highestEntityIndex);
    }
}
